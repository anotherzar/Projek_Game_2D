...
public class EndingDialogueManager : MonoBehaviour
{
    ...
    [Tooltip("Nama scene yang akan di-load setelah dialog selesai")]
    public string sceneEnding = "main_menu";
    ...
    void Start()
    {
        ...
        // Play BGM — jika AudioManager sudah memainkan BGM dari scene sebelumnya,
        // biarkan terus jalan (nyambung). Hanya mulai baru jika belum ada musik.
        if (AudioManager.instance != null)
        {
            if (!AudioManager.instance.IsPlaying())
            {
                // Belum ada musik, mulai BGM baru
                if (bgmLevel != null)
                {
                    AudioManager.instance.PlayBGM(bgmLevel, bgmVolume);
                    Debug.Log("[EndingDialogueManager] Memulai BGM baru: " + bgmLevel.name);
                }
            }
            else
            {
                Debug.Log("[EndingDialogueManager] BGM sudah berjalan dari scene sebelumnya, nyambung terus.");
            }
        }
    ...
    void EndDialogue()
    {
        ...
        UpdatePetunjuk();
        StartCoroutine(TransisiKeEndingScene());
    }

    IEnumerator TransisiKeEndingScene()
    {
        Debug.Log("[EndingDialogueManager] Memulai transisi ke scene: " + sceneEnding);
        ...
        if (!sceneValid)
        {
            Debug.LogError("[EndingDialogueManager] Scene '" + sceneEnding + "' TIDAK ditemukan di Build Settings! Tambahkan dulu via File > Build Settings > Add Open Scenes.");
            yield break;
        }

        // Catatan: BGM sengaja tidak di-stop di sini agar bisa nyambung terus ke scene berikutnya
        // selama AudioManager masih hidup via DontDestroyOnLoad
        ...
        UnityEngine.SceneManagement.SceneManager.LoadScene(sceneEnding);
    }
